...
namespace Entidades
{
    public class ConexionSQL
    {
        ...
        public void GuardarProducto(Producto info)
        {
            try
            {
                string command = "INSERT INTO Producto (codigo,nombre, precio,  tipo) " +
                    "VALUES (@codigo, @nombre,@precio,@tipo)";
        ...
        public void GuardarVenta(Venta info)
        {
            try
            {
                string command = "INSERT INTO Venta (codigo,nombreCliente, concepto,  producto) " +
                    "VALUES (@codigo, @nombreCliente,@concepto,@producto)";
                ...
                sqlCommand.Parameters.AddWithValue("concepto", info.Concepto);
                sqlCommand.Parameters.AddWithValue("producto", info.Producto.Codigo);
        ...
        public Venta LeerVenta()
        {
            ...
            try
            {
                ...
                while (read.Read()) //Devuelve true si hay filas, si pudo leer
                {
                    ...
                    datos.Codigo = read.GetInt32(0);
                    datos.NombreCliente = read.GetString(1);
                    datos.Concepto = (Venta.ConceptoDePago) read.GetInt32(2);
                    datos.Codigo = read.GetInt32(3);
                }
            }
            finally
            {
