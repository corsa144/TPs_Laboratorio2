...
namespace Entidades
{
    public class ConexionSQL
    {
        ...
        public void GuardarProducto(Producto info)
        {
            try
            {
                string command = "INSERT INTO Producto (nombre, precio) " +
        ...
        public List<Producto> LeerProducto()
        {
            ...
            try
            {
                ...
                while (read.Read()) //Devuelve true si hay filas, si pudo leer
                {
                    ...
                    p.Codigo = read.GetInt32(0);
                    p.Nombre = read.GetString(1);
                    p.Precio = read.GetDouble(2);
                    productos.Add(p);
                }
            }
            finally
        ...
        public void GuardarVenta(Venta info)
        {
            try
            {
                string command = "INSERT INTO Venta (nombre) " +
                    "VALUES (@nombre)";
                ...
                sqlCommand.Parameters.AddWithValue("nombre", info.NombreCliente);
               // sqlCommand.Parameters.AddWithValue("codigo", info.Codigo);
